...
VERILATOR_OUTPUT=$(verilator -Wall --cc --exe -I"$INCLUDE_DIR" simulator.cpp DevelopmentBoard.v \
 -LDFLAGS "-lglut -lGLU -lGL -pthread")
...
make -j -C obj_dir -f VDevelopmentBoard.mk VDevelopmentBoard LDFLAGS="-pthread"
# 检查make是否成功构建
if [ $? -ne 0 ]; then
